...
- **盘古空格**：中英混排时在汉字与英文/数字相邻处自动补半角空格（词组只在两端补，如「使用 GPT 模型」）；预览 / SRT 导出 / 烧录实时生效，不改字幕原文，空格不计入每行字数上限；「设置 → 断句」可关
- **手动编辑**：字幕条纯增删改（新增 / 编辑文本 / 修改时间 / 删除）
- **播放控制**：视频下方常驻进度条（贴视频下边缘、与视频同宽，点击跳转）；−n帧 / +n帧 步进（帧数在控制栏内可编辑并持久化，按住连发，快捷键 `,` / `.`，步长取视频真实帧率，自动暂停）；暂停时左下角悬浮「继续播放」按钮；跳转 / 逐帧实时同步字幕与列表
